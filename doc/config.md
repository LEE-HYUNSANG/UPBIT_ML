...
List of dictionaries continuously updated by **F2** for each monitored coin.
Each entry contains `symbol`, `ml_signal`, `rsi_sel`, `trend_sel`, `buy_signal`,
and `buy_count`. `ml_signal` records the pure ML prediction (0 or 1), while
`buy_signal` is set to `1` only when all other flags are `1`.
